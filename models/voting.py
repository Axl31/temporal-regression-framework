"""
Voting ensemble wrapper using sklearn VotingRegressor over combinations of base learners.
"""
...
class Voting(BaseModelWrapper):
    """
    Voting ensemble using all possible combinations of base regressors.
    """

    def tuning(self):
        """
        Fit VotingRegressor for all combinations of base learners, compute RMSE, and track time.

        Returns:
            models (list): list of fitted VotingRegressor models
        ...
        for idx, combo in enumerate(all_combinations):
            ...
            voting_model.fit(self.X, self.y)

            # Predict and compute RMSE
            pred = voting_model.predict(self.testX)
        ...
        print(f"The best ensemble is combination {best_idx+1} with RMSE {errors[best_idx]:.6f}")
